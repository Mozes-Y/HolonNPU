#!/usr/bin/env python3
"""Run the verified guest Transformer and autonomous timing sensitivity gates."""
import argparse
import json
import shutil
import subprocess
...
def main():
    ...
    for name, latency, lanes, bandwidth in cases:
        ...
        if result["reason"] != "stopped" or result["status"] or result["traps"]:
            raise ValueError(f"{name}: unexpected execution outcome {result}")
        stats = {}
        ...
        if ticks != result["ticks"] or stats["elapsedTicks::memory_wait"] != stats["memoryTicks"]:
            raise ValueError(f"{name}: elapsed-time ledger does not reconcile with execution")
    ...
    print("Autonomous gem5 Transformer and timing sensitivity PASS")
