#include "Vnpu_v2_completion_writer.h"
...
void eval(Vnpu_v2_completion_writer& dut) {
    dut.eval();
}

void tick(Vnpu_v2_completion_writer& dut) {
    dut.clk_i = 0;
    eval(dut);
    dut.clk_i = 1;
    eval(dut);
...
void clear_inputs(Vnpu_v2_completion_writer& dut) {
    dut.soft_reset_i = 0;
    dut.start_i = 0;
...
void reset(Vnpu_v2_completion_writer& dut) {
    dut.clk_i = 0;
    dut.rst_ni = 0;
    clear_inputs(dut);
    tick(dut);
...
Beat accept_write_beat(Vnpu_v2_completion_writer& dut) {
...
std::array<Beat, 2> accept_address_and_data(Vnpu_v2_completion_writer& dut, bool& ok) {
...
bool test_success_record(Vnpu_v2_completion_writer& dut) {
    reset(dut);
    start_write(dut, 0x100, false, HOLON_NPU_V2_FAULT_EXPLICIT_PROGRAM_FAULT);

    bool ok = true;
    const auto beats = accept_address_and_data(dut, ok);
    ok &= expect_eq("beat0 ABI", beats[0].lo & 0xFFFF'FFFFULL,
                    HOLON_NPU_V2_ABI_VERSION_RESET);
...
bool test_fault_record(Vnpu_v2_completion_writer& dut) {
    reset(dut);
    start_write(dut, 0x100, true, HOLON_NPU_V2_FAULT_EXPLICIT_PROGRAM_FAULT);
...
bool test_write_response_fault(Vnpu_v2_completion_writer& dut) {
    reset(dut);
    ...
    ok &= expect_eq("BRESP fault code", dut.fault_code_o, HOLON_NPU_V2_FAULT_AXI_WRITE);
    return ok;
}

bool test_invalid_address(Vnpu_v2_completion_writer& dut) {
    reset(dut);
...
int main(int argc, char** argv) {
    holon_npu_tb::test_run test{"npu_v2_completion_writer", argc, argv};
    Vnpu_v2_completion_writer dut;

    bool ok = true;
    ok &= test_success_record(dut);
    ok &= test_fault_record(dut);
    ok &= test_write_response_fault(dut);
