...
void eval(Vnpu_matrix_engine& dut) {
    dut.eval();
}

void tick(Vnpu_matrix_engine& dut) {
    dut.clk_i = 0;
    eval(dut);
    dut.clk_i = 1;
    eval(dut);
...
void clear_inputs(Vnpu_matrix_engine& dut) {
    dut.soft_reset_i = 0;
...
void reset(Vnpu_matrix_engine& dut) {
    dut.clk_i = 0;
    dut.rst_ni = 0;
    clear_inputs(dut);
    tick(dut);
...
bool host_write_byte(Vnpu_matrix_engine& dut, std::uint32_t addr, std::uint8_t data) {
...
read_result host_read(Vnpu_matrix_engine& dut, std::uint32_t addr) {
...
bool write_command(Vnpu_matrix_engine& dut, std::uint8_t flags, std::uint8_t m = 2) {
...
bool expect_event_ok(std::string_view name, const event_result& event) {
    ...
    ok &= expect_eq(std::string{name} + " code", event.fault_code, HOLON_NPU_FAULT_NONE);
    return ok;
}

bool expect_c(Vnpu_matrix_engine& dut, std::span<const std::int32_t> expected) {
    bool ok = true;
...
bool test_clear_accumulate_store(Vnpu_matrix_engine& dut) {
    reset(dut);
    bool ok = true;
...
bool test_matrix_faults(Vnpu_matrix_engine& dut) {
    reset(dut);
    ...
    ok &= expect_eq("bad shape code", bad_shape.fault_code, HOLON_NPU_FAULT_MATRIX_ISSUE);
    ...
    ok &= expect_eq("unaligned code", unaligned.fault_code, HOLON_NPU_FAULT_MATRIX_ISSUE);
    return ok;
}
...
bool test_constrained_random(Vnpu_matrix_engine& dut) {
...
int main(int argc, char** argv) {
    holon_npu_tb::test_run test{"npu_matrix_engine", argc, argv};
    Vnpu_matrix_engine dut;
    const bool datapath_case = test_clear_accumulate_store(dut);
    test.observe(
        {
            holon_npu_tb::coverage_point::matrix_engine_clear,
            holon_npu_tb::coverage_point::matrix_engine_accumulate,
            holon_npu_tb::coverage_point::matrix_engine_store,
            holon_npu_tb::coverage_point::isa_class_matrix,
            holon_npu_tb::coverage_point::matrix_gemm_program,
        },
        datapath_case
    );
    bool ok = datapath_case;

    const bool fault_case = test_matrix_faults(dut);
    test.observe(holon_npu_tb::coverage_point::matrix_engine_fault, fault_case);
    ok &= fault_case;

    const bool random_case = test_constrained_random(dut);
    test.observe(holon_npu_tb::coverage_point::matrix_constrained_random, random_case);
    ok &= random_case;
    dut.final();
