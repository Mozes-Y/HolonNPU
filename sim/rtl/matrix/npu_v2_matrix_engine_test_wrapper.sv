...
module npu_v2_matrix_engine_test_wrapper (
    input  logic         clk_i,
    input  logic         rst_ni,
    ...
    input  logic [3:0]   host_wr_strb_i,
    ...
    npu_vr_if #(.DATA_W(128)) issue_if (.clk_i(clk_i), .rst_ni(rst_ni));
    npu_vr_if #(.DATA_W(64)) event_if (.clk_i(clk_i), .rst_ni(rst_ni));
    npu_v2_localmem_wr_if host_wr_if (.clk_i(clk_i), .rst_ni(rst_ni));
    npu_v2_localmem_wr_if matrix_wr_if (.clk_i(clk_i), .rst_ni(rst_ni));
    npu_v2_localmem_wr_if memory_wr_if (.clk_i(clk_i), .rst_ni(rst_ni));
    npu_v2_localmem_wr_if program_wr_if (.clk_i(clk_i), .rst_ni(rst_ni));
    npu_v2_localmem_rd_if host_rd_if (.clk_i(clk_i), .rst_ni(rst_ni));
    npu_v2_localmem_rd_if matrix_rd_if (.clk_i(clk_i), .rst_ni(rst_ni));
    npu_v2_localmem_rd_if memory_rd_if (.clk_i(clk_i), .rst_ni(rst_ni));
    npu_v2_localmem_rd_if program_rd_if (.clk_i(clk_i), .rst_ni(rst_ni));
    ...
    assign host_wr_if.req_strb = host_wr_strb_i;
    ...
    npu_v2_matrix_engine_core matrix_engine (
        .clk_i(clk_i),
        .rst_ni(rst_ni),
        ...
        .data_rd(matrix_rd_if.master),
        .data_wr(matrix_wr_if.master)
    );
    ...
    npu_v2_data_port_arbiter_core data_arbiter (
        ...
        .client_wr(matrix_wr_if.slave),
        .memory_wr(memory_wr_if.master),
        .host_rd(host_rd_if.slave),
        .client_rd(matrix_rd_if.slave),
        .memory_rd(memory_rd_if.master)
    );
