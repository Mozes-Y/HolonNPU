...
void eval(Vnpu_loader& dut) {
    dut.eval();
}

void tick(Vnpu_loader& dut) {
    dut.clk_i = 0;
    eval(dut);
    dut.clk_i = 1;
    eval(dut);
...
void clear_inputs(Vnpu_loader& dut) {
    dut.soft_reset_i = 0;
    dut.start_i = 0;
    dut.desc_addr_i = 0;
...
void start_loader(Vnpu_loader& dut, std::uint64_t desc_addr) {
...
bool run_until_terminal(Vnpu_loader& dut, DescriptorMemory& memory, int max_cycles = 80) {
...
bool test_valid_descriptor(Vnpu_loader& dut) {
    reset(dut);
...
bool test_zero_argument_descriptor(Vnpu_loader& dut) {
    reset(dut);
    DescriptorMemory memory(0x8000);
...
bool test_4k_boundary_splits(Vnpu_loader& dut) {
    constexpr std::uint64_t descriptor_addr = 0x0FF0;
    reset(dut);
    DescriptorMemory memory(0x5000);
    auto desc = valid_descriptor();
    desc.code_addr = 0x1FFC;
    desc.code_size_bytes = 32;
    desc.program_mem_bytes = 32;
    desc.arg_addr = 0x2FF0;
    desc.arg_size_bytes = 32;
    desc.local_mem_bytes = 32;
    desc.completion_addr = 0x3FF0;
    const std::array<std::uint32_t, 8> program_words{
        0x1000'0001U, 0x1000'0002U, 0x1000'0003U, 0x1000'0004U,
        0x1000'0005U, 0x1000'0006U, 0x1000'0007U, 0x1000'0008U,
    };
    const std::array<std::uint32_t, 8> argument_words{
        0x2000'0001U, 0x2000'0002U, 0x2000'0003U, 0x2000'0004U,
        0x2000'0005U, 0x2000'0006U, 0x2000'0007U, 0x2000'0008U,
    };
    memory.store(descriptor_addr, desc);
    memory.store_words(desc.code_addr, program_words);
    memory.store_words(desc.arg_addr, argument_words);

    start_loader(dut, descriptor_addr);
    bool ok = run_until_terminal(dut, memory, 240);
    ok &= expect_eq("4 KiB split done", dut.done_o, 1U);
    ok &= expect_eq("4 KiB split fault", dut.fault_o, 0U);
    ok &= expect_eq("4 KiB split burst count", memory.observed_bursts().size(), 6U);
    ok &= expect_burst(memory.observed_bursts(), 0, 0x0FF0, 1, 4);
    ok &= expect_burst(memory.observed_bursts(), 1, 0x1000, 7, 4);
    ok &= expect_burst(memory.observed_bursts(), 2, 0x1FFC, 1, 2);
    ok &= expect_burst(memory.observed_bursts(), 3, 0x2000, 7, 2);
    ok &= expect_burst(memory.observed_bursts(), 4, 0x2FF0, 4, 2);
    ok &= expect_burst(memory.observed_bursts(), 5, 0x3000, 4, 2);
    ok &= expect_writes("4 KiB program writes", memory.program_writes(), program_words);
    ok &= expect_writes("4 KiB argument writes", memory.data_writes(), argument_words);
    return ok;
}
...
bool test_descriptor_validation_faults(Vnpu_loader& dut) {
    bool ok = true;
    auto desc = valid_descriptor();

    desc.size_bytes = 64;
    ok &= expect_descriptor_fault(dut, "bad size", desc, HOLON_NPU_FAULT_INVALID_PROGRAM_DESCRIPTOR);

    desc = valid_descriptor();
    desc.version = 2;
    ok &= expect_descriptor_fault(dut, "bad version", desc, HOLON_NPU_FAULT_UNSUPPORTED_ABI_OR_ISA);

    desc = valid_descriptor();
    desc.holon_isa_major = 2;
    ok &= expect_descriptor_fault(dut, "bad ISA major", desc, HOLON_NPU_FAULT_UNSUPPORTED_ABI_OR_ISA);

    desc = valid_descriptor();
    desc.holon_isa_minor = static_cast<std::uint16_t>(HOLON_NPU_ISA_MINOR + 1U);
    ok &= expect_descriptor_fault(dut, "bad ISA minor", desc, HOLON_NPU_FAULT_UNSUPPORTED_ABI_OR_ISA);

    desc = valid_descriptor();
    desc.program_format = 0;
    ok &= expect_descriptor_fault(dut, "bad program format", desc, HOLON_NPU_FAULT_UNSUPPORTED_PROGRAM_FORMAT);

    desc = valid_descriptor();
    desc.required_caps = std::uint64_t{1} << 63U;
    ok &= expect_descriptor_fault(dut, "bad capability", desc, HOLON_NPU_FAULT_UNSUPPORTED_CAPABILITY);

    desc = valid_descriptor();
    desc.required_op_classes = std::uint64_t{1} << 63U;
    ok &= expect_descriptor_fault(dut, "bad op class", desc, HOLON_NPU_FAULT_UNSUPPORTED_OPERATION_CLASS);

    desc = valid_descriptor();
    desc.flags = HOLON_NPU_PROGRAM_FLAG_VALID_MASK << 1U;
    ok &= expect_descriptor_fault(dut, "bad flags", desc, HOLON_NPU_FAULT_INVALID_PROGRAM_DESCRIPTOR);

    desc = valid_descriptor();
    desc.reserved_50 = 1;
    ok &= expect_descriptor_fault(dut, "bad reserved", desc, HOLON_NPU_FAULT_INVALID_PROGRAM_DESCRIPTOR);

    desc = valid_descriptor();
    desc.code_addr = 0x2002;
    ok &= expect_descriptor_fault(dut, "bad code alignment", desc, HOLON_NPU_FAULT_ALIGNMENT);

    desc = valid_descriptor();
    desc.arg_size_bytes = 4;
    ok &= expect_descriptor_fault(dut, "bad argument size alignment", desc, HOLON_NPU_FAULT_ALIGNMENT);

    desc = valid_descriptor();
    desc.code_size_bytes = 0;
    ok &= expect_descriptor_fault(dut, "zero code size", desc, HOLON_NPU_FAULT_LOCAL_MEMORY_BOUNDS);

    desc = valid_descriptor();
    desc.entry_pc = desc.code_size_bytes;
    ok &= expect_descriptor_fault(dut, "entry out of bounds", desc, HOLON_NPU_FAULT_LOCAL_MEMORY_BOUNDS);

    desc = valid_descriptor();
    desc.program_mem_bytes = desc.code_size_bytes - 4U;
    ok &= expect_descriptor_fault(dut, "program memory too small", desc, HOLON_NPU_FAULT_LOCAL_MEMORY_BOUNDS);

    desc = valid_descriptor();
    desc.local_mem_bytes = desc.arg_size_bytes - 4U;
    ok &= expect_descriptor_fault(dut, "argument memory too small", desc, HOLON_NPU_FAULT_LOCAL_MEMORY_BOUNDS);

    desc = valid_descriptor();
    desc.local_mem_bytes = HOLON_NPU_LOCAL_MEM_MAX_BYTES + 16U;
    ok &= expect_descriptor_fault(dut, "local memory too large", desc, HOLON_NPU_FAULT_LOCAL_MEMORY_BOUNDS);

    desc = valid_descriptor();
    desc.stack_bytes = desc.local_mem_bytes - desc.arg_size_bytes + 1U;
    ok &= expect_descriptor_fault(dut, "argument and stack overlap", desc, HOLON_NPU_FAULT_LOCAL_MEMORY_BOUNDS);

    desc = valid_descriptor();
    desc.code_addr = std::numeric_limits<std::uint64_t>::max() - 15U;
    ok &= expect_descriptor_fault(dut, "code address overflow", desc, HOLON_NPU_FAULT_INVALID_PROGRAM_DESCRIPTOR);

    desc = valid_descriptor();
    desc.arg_addr = std::numeric_limits<std::uint64_t>::max() - 15U;
    ok &= expect_descriptor_fault(dut, "argument address overflow", desc, HOLON_NPU_FAULT_INVALID_PROGRAM_DESCRIPTOR);

    desc = valid_descriptor();
    desc.completion_addr = std::numeric_limits<std::uint64_t>::max() - 15U;
    ok &= expect_descriptor_fault(dut, "completion address overflow", desc, HOLON_NPU_FAULT_INVALID_PROGRAM_DESCRIPTOR);

    return ok;
}

bool test_descriptor_address_alignment(Vnpu_loader& dut) {
    reset(dut);
    DescriptorMemory memory(0x8000);
    ...
    ok &= expect_eq("unaligned desc fault code", dut.fault_code_o, HOLON_NPU_FAULT_ALIGNMENT);
    ok &= expect_eq("unaligned desc no AXI", memory.observed_bursts().size(), 0);
    return ok;
}

bool test_descriptor_address_overflow(Vnpu_loader& dut) {
    reset(dut);
    ...
    ok &= expect_eq("overflow desc fault code", dut.fault_code_o, HOLON_NPU_FAULT_AXI_READ);
    ok &= expect_eq("overflow desc no AXI", memory.observed_bursts().size(), 0);
    return ok;
}

bool test_axi_error(Vnpu_loader& dut) {
    reset(dut);
    DescriptorMemory memory(0x8000);
    memory.store(kDescAddr, valid_descriptor());
    ...
    ok &= expect_eq("AXI read fault code", dut.fault_code_o, HOLON_NPU_FAULT_AXI_READ);
...
int main(int argc, char** argv) {
    holon_npu_tb::test_run test{"npu_loader", argc, argv};

    Vnpu_loader dut;
    bool ok = true;

    using enum holon_npu_tb::coverage_point;
    const bool valid_case = test_valid_descriptor(dut);
    test.observe(
        {descriptor_valid, loader_program_image_copy, loader_argument_copy},
        valid_case
    );
    ok &= valid_case;

    const bool zero_argument_case = test_zero_argument_descriptor(dut);
    test.observe(loader_zero_argument_skip, zero_argument_case);
    ok &= zero_argument_case;

    const bool boundary_case = test_4k_boundary_splits(dut);
    test.observe({loader_descriptor_4k_split, loader_copy_4k_split}, boundary_case);
    ok &= boundary_case;

    const bool validation_case = test_descriptor_validation_faults(dut);
    test.observe(
        {
    ...
    const bool overflow_case = test_descriptor_address_overflow(dut);
    test.observe(descriptor_address_overflow, overflow_case);
    ok &= overflow_case;

    const bool axi_error_case = test_axi_error(dut);
    test.observe(loader_axi_read_error, axi_error_case);
    ok &= axi_error_case;
