#include "Vnpu_v2_local_memory.h"
...
void eval(Vnpu_v2_local_memory& dut) {
    dut.eval();
}

void tick(Vnpu_v2_local_memory& dut) {
    dut.clk_i = 0;
    eval(dut);
    dut.clk_i = 1;
    eval(dut);
...
void clear_inputs(Vnpu_v2_local_memory& dut) {
    dut.soft_reset_i = 0;
    ...
    dut.client_data_rd_valid_i = 0;
    dut.client_data_rd_addr_i = 0;
}

void reset(Vnpu_v2_local_memory& dut) {
    dut.clk_i = 0;
    dut.rst_ni = 0;
    clear_inputs(dut);
    tick(dut);
...
bool program_write(Vnpu_v2_local_memory& dut, std::uint32_t addr, std::uint32_t data) {
...
bool client_data_write(Vnpu_v2_local_memory& dut, std::uint32_t addr, std::uint32_t data) {
...
ReadResult program_read(Vnpu_v2_local_memory& dut, std::uint32_t addr) {
...
ReadResult data_read(Vnpu_v2_local_memory& dut, std::uint32_t addr) {
...
ReadResult client_data_read(Vnpu_v2_local_memory& dut, std::uint32_t addr) {
    dut.client_data_rd_valid_i = 1;
    dut.client_data_rd_addr_i = addr;
    eval(dut);
    const bool ready = dut.client_data_rd_ready_o != 0;
    tick(dut);
    ReadResult result{
        .valid = dut.client_data_rd_valid_o != 0,
        .ready = ready,
        .error = dut.client_data_rd_error_o != 0,
        .data = static_cast<std::uint32_t>(dut.client_data_rd_data_o),
    };
    dut.client_data_rd_valid_i = 0;
    tick(dut);
    return result;
...
bool test_program_and_data_storage(Vnpu_v2_local_memory& dut) {
    reset(dut);
...
bool test_bounds_and_alignment_faults(Vnpu_v2_local_memory& dut) {
    reset(dut);
...
bool test_data_port_arbitration(Vnpu_v2_local_memory& dut) {
    reset(dut);
    ...
    const auto client_word = client_data_read(dut, 12);
    ...
    dut.data_rd_valid_i = 1;
    dut.data_rd_addr_i = 8;
    dut.client_data_rd_valid_i = 1;
    dut.client_data_rd_addr_i = 12;
    eval(dut);
    ok &= expect_eq("host read first", dut.data_rd_ready_o, 1);
    ok &= expect_eq("client read waits", dut.client_data_rd_ready_o, 0);
    tick(dut);
    ok &= expect_eq("host read response valid", dut.data_rd_valid_o, 1);
    ok &= expect_eq("host read response data", static_cast<std::uint32_t>(dut.data_rd_data_o), 0xAAAA'0008U);

    dut.data_rd_valid_i = 0;
    eval(dut);
    ok &= expect_eq("client read accepted next", dut.client_data_rd_ready_o, 1);
    tick(dut);
    ok &= expect_eq("client read response valid", dut.client_data_rd_valid_o, 1);
    ok &= expect_eq("client read response data", static_cast<std::uint32_t>(dut.client_data_rd_data_o), 0xBBBB'000CU);
...
int main(int argc, char** argv) {
    holon_npu_tb::test_run test{"npu_v2_local_memory", argc, argv};

    Vnpu_v2_local_memory dut;
    bool ok = true;
    ok &= test_program_and_data_storage(dut);
    ok &= test_bounds_and_alignment_faults(dut);
    ok &= test_data_port_arbitration(dut);
