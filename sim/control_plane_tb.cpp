...
void eval(Vnpu_control_plane& dut) {
    dut.eval();
}

void tick(Vnpu_control_plane& dut) {
    dut.clk_i = 0;
    eval(dut);
    dut.clk_i = 1;
    eval(dut);
...
void clear_inputs(Vnpu_control_plane& dut) {
...
void idle_axi_memory_bus(Vnpu_control_plane& dut) {
    dut.m_axi_arready_i = 0;
    dut.m_axi_rdata_lo_i = 0;
    dut.m_axi_rdata_hi_i = 0;
    dut.m_axi_rresp_i = kRespOkay;
...
void reset(Vnpu_control_plane& dut) {
    dut.clk_i = 0;
    dut.rst_ni = 0;
    clear_inputs(dut);
    tick(dut);
...
public:
    ...
    void step(Vnpu_control_plane& dut) {
...
std::uint32_t axil_write(Vnpu_control_plane& dut, std::uint32_t addr, std::uint32_t data) {
...
std::uint32_t axil_read(Vnpu_control_plane& dut, std::uint32_t addr) {
...
void frontend_done(Vnpu_control_plane& dut, std::uint32_t pc, std::uint64_t instret) {
...
ReadResult program_read(Vnpu_control_plane& dut, std::uint32_t addr) {
...
bool expect_local_words(
    ...
    for (std::size_t index = 0; index < expected_words.size(); ++index) {
        ...
        const auto result = program_read(dut, addr);
...
bool run_until_loader_terminal(Vnpu_control_plane& dut, AxiMemory& memory, int max_cycles = 160) {
...
bool test_valid_program_launch(Vnpu_control_plane& dut) {
    reset(dut);
    ...
    ok &= expect_local_words(dut, "program readback", program_words);
    ...
    ok &= expect_eq("debug pc", axil_read(dut, HOLON_NPU_REG_DEBUG_PC), 0x40U);
    ok &= expect_eq("instret", axil_read(dut, HOLON_NPU_REG_PERF_INSTRET_LO), 9U);
    ...
    idle_axi_memory_bus(dut);
    ok &= expect_eq("restart doorbell", axil_write(dut, HOLON_NPU_REG_DOORBELL, 1), kOkay);
    ok &= expect_eq("restart terminal", run_until_loader_terminal(dut, memory), true);
    ok &= expect_eq("restart status running", axil_read(dut, HOLON_NPU_REG_STATUS), HOLON_NPU_STATUS_RUNNING);
    ok &= expect_local_words(dut, "restart program readback", program_words);
    return ok;
}

bool test_loader_fault_propagates(Vnpu_control_plane& dut) {
    reset(dut);
    AxiMemory memory(0x8000);
    ...
    ok &= expect_eq("loader fault code", dut.loader_fault_code_o, HOLON_NPU_FAULT_UNSUPPORTED_ABI_OR_ISA);
    ok &= expect_eq("status fault", axil_read(dut, HOLON_NPU_REG_STATUS), HOLON_NPU_STATUS_FAULT);
    ok &= expect_eq("fault code", axil_read(dut, HOLON_NPU_REG_FAULT_CODE),
                    HOLON_NPU_FAULT_UNSUPPORTED_ABI_OR_ISA);
...
int main(int argc, char** argv) {
    holon_npu_tb::test_run test{"npu_control_plane", argc, argv};

    Vnpu_control_plane dut;
    bool ok = true;

    using enum holon_npu_tb::coverage_point;
    const bool launch_case = test_valid_program_launch(dut);
    test.observe(
        {
    ...
    const bool fault_case = test_loader_fault_propagates(dut);
    test.observe(descriptor_invalid_version, fault_case);
    ok &= fault_case;
