...
void eval(Vnpu_vector_engine& dut) {
    dut.eval();
}

void tick(Vnpu_vector_engine& dut) {
    dut.clk_i = 0;
    eval(dut);
    dut.clk_i = 1;
    eval(dut);
...
void clear_inputs(Vnpu_vector_engine& dut) {
    dut.soft_reset_i = 0;
...
void reset(Vnpu_vector_engine& dut) {
    dut.clk_i = 0;
    dut.rst_ni = 0;
    clear_inputs(dut);
    tick(dut);
...
bool host_write(Vnpu_vector_engine& dut, std::uint32_t addr, std::uint32_t data) {
...
ReadResult host_read(Vnpu_vector_engine& dut, std::uint32_t addr) {
...
bool expect_ok_event(std::string_view name, const EventResult& event) {
    ...
    ok &= expect_eq(std::string{name} + " fault code", event.fault_code, HOLON_NPU_FAULT_NONE);
    return ok;
}
...
bool expect_memory_word(Vnpu_vector_engine& dut, std::string_view name, std::uint32_t addr, std::uint32_t expected) {
...
bool load_operands(Vnpu_vector_engine& dut, std::uint32_t lhs_base, std::uint32_t rhs_base) {
    bool ok = true;
...
bool test_vector_load_store_and_add(Vnpu_vector_engine& dut) {
    reset(dut);
...
bool test_vector_alu_compare_shift(Vnpu_vector_engine& dut) {
    reset(dut);
...
bool test_narrow_elements(Vnpu_vector_engine& dut) {
    reset(dut);
...
bool test_predicate_execution(Vnpu_vector_engine& dut) {
    reset(dut);
...
bool test_saturating_arithmetic(Vnpu_vector_engine& dut) {
    reset(dut);
...
bool test_pack_unpack_and_transpose(Vnpu_vector_engine& dut) {
    reset(dut);
    ...
    ok &= expect_fault_event(
        "odd zip fault",
        issue(dut, vector_permute(HOLON_NPU_ISA_OPCODE_VECTOR_PERMUTE_ZIP_LO, 3, 1, 2)),
        HOLON_NPU_FAULT_VECTOR_CONFIG
    );
    return ok;
}

bool test_select_and_gather(Vnpu_vector_engine& dut) {
    reset(dut);
...
bool test_reduction_identities(Vnpu_vector_engine& dut) {
    reset(dut);
...
bool test_requantize(Vnpu_vector_engine& dut) {
    reset(dut);
...
bool test_vector_faults(Vnpu_vector_engine& dut) {
    reset(dut);
    ...
    ok &= expect_fault_event(
        ...
        HOLON_NPU_FAULT_LOCAL_MEMORY_BOUNDS
    );
    ok &= expect_fault_event(
...
int main(int argc, char** argv) {
    holon_npu_tb::test_run test{"npu_vector_engine", argc, argv};
    Verilated::traceEverOn(false);

    Vnpu_vector_engine dut;
    bool ok = true;
    using enum holon_npu_tb::coverage_point;

    const bool load_store_case = test_vector_load_store_and_add(dut);
    test.observe(
        {
            holon_npu_tb::coverage_point::vector_config,
            holon_npu_tb::coverage_point::vector_load_store,
            holon_npu_tb::coverage_point::vector_alu,
            ...
            holon_npu_tb::coverage_point::vector_config_set,
            holon_npu_tb::coverage_point::vector_load,
            holon_npu_tb::coverage_point::vector_store,
            holon_npu_tb::coverage_point::vector_alu_add,
        },
        load_store_case
    );
    ok &= load_store_case;

    const bool alu_case = test_vector_alu_compare_shift(dut);
    test.observe(
        {
            vector_compare_shift,
            holon_npu_tb::coverage_point::vector_alu_sub,
            holon_npu_tb::coverage_point::vector_alu_min,
            holon_npu_tb::coverage_point::vector_alu_max,
            holon_npu_tb::coverage_point::vector_alu_eq,
            holon_npu_tb::coverage_point::vector_alu_lt,
            holon_npu_tb::coverage_point::vector_alu_shl,
            holon_npu_tb::coverage_point::vector_alu_srl,
            holon_npu_tb::coverage_point::vector_alu_sra,
        },
        alu_case
    );
    ok &= alu_case;

    const bool narrow_case = test_narrow_elements(dut);
    test.observe(vector_narrow_elements, narrow_case);
    ok &= narrow_case;

    const bool predicate_case = test_predicate_execution(dut);
    test.observe(
        {vector_predicate, isa_class_predicate, predicate_ptrue, predicate_load},
        predicate_case
    );
    ok &= predicate_case;

    const bool saturation_case = test_saturating_arithmetic(dut);
    test.observe(vector_saturating_arithmetic, saturation_case);
    ok &= saturation_case;

    const bool select_case = test_select_and_gather(dut);
    test.observe(
        {
            vector_select,
            vector_permute,
            holon_npu_tb::coverage_point::vector_alu_select,
            holon_npu_tb::coverage_point::vector_permute_gather,
        },
        select_case
    );
    ok &= select_case;

    const bool pack_case = test_pack_unpack_and_transpose(dut);
    test.observe(
        {
            vector_pack_unpack,
            vector_transpose,
            isa_class_vector_permute,
            holon_npu_tb::coverage_point::vector_permute_zip_lo,
            holon_npu_tb::coverage_point::vector_permute_zip_hi,
            holon_npu_tb::coverage_point::vector_permute_unzip_even,
            holon_npu_tb::coverage_point::vector_permute_unzip_odd,
            holon_npu_tb::coverage_point::vector_permute_transpose4,
        },
        pack_case
    );
    ok &= pack_case;

    const bool reduction_case = test_reduction_identities(dut);
    test.observe(
        {
            vector_reduction,
            isa_class_vector_reduction,
            holon_npu_tb::coverage_point::vector_reduce_sum,
            holon_npu_tb::coverage_point::vector_reduce_min,
            holon_npu_tb::coverage_point::vector_reduce_max,
        },
        reduction_case
    );
    ok &= reduction_case;

    const bool requant_case = test_requantize(dut);
    test.observe(
        {
            vector_requant,
            isa_class_quantization,
            holon_npu_tb::coverage_point::quantization_requantize,
        },
        requant_case
    );
    ok &= requant_case;

    const bool fault_case = test_vector_faults(dut);
    test.observe(vector_faults, fault_case);
    ok &= fault_case;
