...
bool run_vector_add() {
    ...
    ok &= expect((image.required_caps & HOLON_NPU_V2_CAP_INTEGER_VECTOR_BASE) != 0,
...
bool run_requant() {
    ...
    ok &= expect((image.required_caps & HOLON_NPU_V2_CAP_QUANT_VECTOR) != 0,
...
bool run_int8_gemm() {
    ...
    ok &= expect((image.required_caps & HOLON_NPU_V2_CAP_MATRIX_MICRO_OP) != 0,
...
bool test_dma_encoding_contract() {
    const auto maximum = holon_npu::v2::model::decode(
