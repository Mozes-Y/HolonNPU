...
static_assert(HOLON_NPU_PROGRAM_FORMAT_HOLON_V2 == 1U);
...
static_assert(HOLON_NPU_V2_DOORBELL_START == 1U);
...
void reset_regs(RegFile& regs) {
    regs.fill(0);
    regs.at(idx(HOLON_NPU_V2_REG_DEVICE_ID)) = HOLON_NPU_V2_RESET_DEVICE_ID;
    regs.at(idx(HOLON_NPU_V2_REG_ABI_VERSION)) = HOLON_NPU_V2_RESET_ABI_VERSION;
    regs.at(idx(HOLON_NPU_V2_REG_ISA_VERSION)) = HOLON_NPU_V2_RESET_ISA_VERSION;
    regs.at(idx(HOLON_NPU_V2_REG_CAP0_LO)) = HOLON_NPU_V2_RESET_CAP0_LO;
    regs.at(idx(HOLON_NPU_V2_REG_CAP0_HI)) = HOLON_NPU_V2_RESET_CAP0_HI;
    regs.at(idx(HOLON_NPU_V2_REG_OP_CLASS_LO)) = HOLON_NPU_V2_RESET_OP_CLASS_LO;
    regs.at(idx(HOLON_NPU_V2_REG_OP_CLASS_HI)) = HOLON_NPU_V2_RESET_OP_CLASS_HI;
    regs.at(idx(HOLON_NPU_V2_REG_PROGRAM_MEM_BYTES)) = HOLON_NPU_V2_RESET_PROGRAM_MEM_BYTES;
    regs.at(idx(HOLON_NPU_V2_REG_LOCAL_MEM_BYTES)) = HOLON_NPU_V2_RESET_LOCAL_MEM_BYTES;
    regs.at(idx(HOLON_NPU_V2_REG_VECTOR_CAP0)) = HOLON_NPU_V2_RESET_VECTOR_CAP0;
    regs.at(idx(HOLON_NPU_V2_REG_MATRIX_CAP0)) = HOLON_NPU_V2_RESET_MATRIX_CAP0;
    regs.at(idx(HOLON_NPU_V2_REG_STATUS)) = HOLON_NPU_V2_STATUS_IDLE;
}
...
bool test_init_and_caps() {
    RegFile regs;
    reset_regs(regs);
    regs.at(idx(HOLON_NPU_V2_REG_CAP0_HI)) = 0x1234'5678U;
    regs.at(idx(HOLON_NPU_V2_REG_OP_CLASS_HI)) = 0x8765'4321U;
    ...
    ok &= expect_eq("device id", caps.device_id, HOLON_NPU_V2_RESET_DEVICE_ID);
    ok &= expect_eq("ABI", caps.abi_version, HOLON_NPU_V2_ABI_VERSION_RESET);
    ok &= expect_eq("ISA", caps.isa_version, HOLON_NPU_V2_RESET_ISA_VERSION);
    ok &= expect_eq(
        "caps64",
        caps.capabilities,
        (std::uint64_t{0x1234'5678U} << 32U) | HOLON_NPU_V2_RESET_CAP0_LO
    );
    ok &= expect_eq(
        "op classes64",
        caps.operation_classes,
        (std::uint64_t{0x8765'4321U} << 32U) | HOLON_NPU_V2_RESET_OP_CLASS_LO
    );
    ...
    ok &= expect_eq("vector caps", caps.vector_cap0, HOLON_NPU_V2_RESET_VECTOR_CAP0);
    ok &= expect_eq("matrix caps", caps.matrix_cap0, HOLON_NPU_V2_RESET_MATRIX_CAP0);
...
bool test_build_descriptor() {
    ...
    ok &= expect_eq("version", desc.version, HOLON_NPU_V2_ABI_MAJOR);
    ok &= expect_eq("format", desc.program_format, HOLON_NPU_PROGRAM_FORMAT_HOLON_V2);
...
bool test_submit_poll_wait() {
    ...
    ok &= expect_eq("desc lo", regs.at(idx(HOLON_NPU_V2_REG_PROGRAM_DESC_ADDR_LO)), 0x9ABC'DEF0U);
    ok &= expect_eq("desc hi", regs.at(idx(HOLON_NPU_V2_REG_PROGRAM_DESC_ADDR_HI)), 0x1234'5678U);
    ok &= expect_eq("doorbell", regs.at(idx(HOLON_NPU_V2_REG_DOORBELL)), HOLON_NPU_V2_DOORBELL_START);
    ...
    regs.at(idx(HOLON_NPU_V2_REG_STATUS)) = HOLON_NPU_V2_STATUS_DONE |
                                             HOLON_NPU_V2_STATUS_IRQ_PENDING;
    ...
    regs.at(idx(HOLON_NPU_V2_REG_STATUS)) = HOLON_NPU_V2_STATUS_FAULT;
...
bool test_lifecycle_irq_fault_perf() {
    ...
    ok &= expect_eq("halt command", regs.at(idx(HOLON_NPU_V2_REG_CONTROL)), HOLON_NPU_V2_CONTROL_HALT);
    regs.at(idx(HOLON_NPU_V2_REG_STATUS)) = HOLON_NPU_V2_STATUS_HALTED;
    ok &= expect_eq("resume", holon_npu_resume(&dev), HOLON_NPU_OK);
    ok &= expect_eq("resume command", regs.at(idx(HOLON_NPU_V2_REG_CONTROL)), HOLON_NPU_V2_CONTROL_RESUME);
    ok &= expect_eq("debug step", holon_npu_debug_step(&dev), HOLON_NPU_OK);
    ok &= expect_eq("debug command", regs.at(idx(HOLON_NPU_V2_REG_CONTROL)), HOLON_NPU_V2_CONTROL_DEBUG_STEP);
    regs.at(idx(HOLON_NPU_V2_REG_STATUS)) = HOLON_NPU_V2_STATUS_DONE;
    ok &= expect_eq("clear terminal", holon_npu_clear_terminal(&dev), HOLON_NPU_OK);
    ok &= expect_eq("clear command", regs.at(idx(HOLON_NPU_V2_REG_CONTROL)), HOLON_NPU_V2_CONTROL_CLEAR_TERMINAL);
    ok &= expect_eq("soft reset", holon_npu_soft_reset(&dev), HOLON_NPU_OK);
    ok &= expect_eq("reset command", regs.at(idx(HOLON_NPU_V2_REG_CONTROL)), HOLON_NPU_V2_CONTROL_SOFT_RESET);

    regs.at(idx(HOLON_NPU_V2_REG_FAULT_CODE)) = HOLON_NPU_V2_FAULT_AXI_WRITE;
    regs.at(idx(HOLON_NPU_V2_REG_DEBUG_PC)) = 0x80U;
    holon_npu_fault_snapshot_t fault{};
    ok &= expect_eq("fault snapshot", holon_npu_get_fault_snapshot(&dev, &fault), HOLON_NPU_OK);
    ok &= expect_eq("fault code", fault.code, HOLON_NPU_V2_FAULT_AXI_WRITE);
    ok &= expect_eq("fault pc", fault.pc, 0x80U);

    const auto irq_mask = HOLON_NPU_V2_IRQ_DONE | HOLON_NPU_V2_IRQ_FAULT;
    ok &= expect_eq("enable IRQ", holon_npu_set_irq_enable(&dev, irq_mask), HOLON_NPU_OK);
    ok &= expect_eq("IRQ enable register", regs.at(idx(HOLON_NPU_V2_REG_IRQ_ENABLE)), irq_mask);
    ok &= expect_eq("bad IRQ enable", holon_npu_set_irq_enable(&dev, 0x10U), HOLON_NPU_E_ARG);
    regs.at(idx(HOLON_NPU_V2_REG_IRQ_STATUS)) = irq_mask | 0x8000'0000U;
    ...
    ok &= expect_eq("IRQ clear register", regs.at(idx(HOLON_NPU_V2_REG_IRQ_CLEAR)), irq_mask);

    regs.at(idx(HOLON_NPU_V2_REG_PERF_CYCLE_LO)) = 0xFFFF'FFF0U;
    regs.at(idx(HOLON_NPU_V2_REG_PERF_CYCLE_HI)) = 1U;
    regs.at(idx(HOLON_NPU_V2_REG_PERF_INSTRET_LO)) = 0x1234U;
    regs.at(idx(HOLON_NPU_V2_REG_PERF_INSTRET_HI)) = 2U;
