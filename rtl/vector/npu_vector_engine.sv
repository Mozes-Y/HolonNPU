...
module npu_vector_engine_core #(
    ...
    task automatic complete_ok();
        begin
            event_fault_q <= 1'b0;
            event_fault_code_q <= NPU_FAULT_NONE;
            state_q <= STATE_EVENT;
        end
    endtask
                ...
                STATE_IDLE: begin
                    if (issue.valid && issue.ready) begin
                        ...
                        if (!instruction_reserved_zero) begin
                            complete_fault(NPU_FAULT_ILLEGAL_INSTRUCTION);
                            ...
                            if (!predicate_ok(instruction_rd) ||
                                ...
                                if (!vl_ok(vl_q) || (instruction_imm != 12'd0)) begin
                                    complete_fault(NPU_FAULT_VECTOR_CONFIG);
                                end else begin
                                    for (int lane_index = 0; lane_index < MAX_VL; lane_index++) begin
                                ...
                                if (!predicate_load_range_ok(instruction_imm)) begin
                                    complete_fault(NPU_FAULT_LOCAL_MEMORY_BOUNDS);
                                end else begin
                                    state_q <= STATE_PRED_LOAD_REQ;
                                end
                            end else begin
