...
module npu_control_regs_core #(
    ...
    localparam logic [ADDR_W-1:0] REG_ISA_VERSION           = ADDR_W'(NPU_REG_ISA_VERSION);
    localparam logic [ADDR_W-1:0] REG_CAP0_LO               = ADDR_W'(NPU_REG_CAP0_LO);
    localparam logic [ADDR_W-1:0] REG_CAP0_HI               = ADDR_W'(NPU_REG_CAP0_HI);
    localparam logic [ADDR_W-1:0] REG_OP_CLASS_LO           = ADDR_W'(NPU_REG_OP_CLASS_LO);
    localparam logic [ADDR_W-1:0] REG_OP_CLASS_HI           = ADDR_W'(NPU_REG_OP_CLASS_HI);
    localparam logic [ADDR_W-1:0] REG_PROGRAM_MEM_BYTES     = ADDR_W'(NPU_REG_PROGRAM_MEM_BYTES);
    localparam logic [ADDR_W-1:0] REG_LOCAL_MEM_BYTES       = ADDR_W'(NPU_REG_LOCAL_MEM_BYTES);
    localparam logic [ADDR_W-1:0] REG_VECTOR_CAP0           = ADDR_W'(NPU_REG_VECTOR_CAP0);
    localparam logic [ADDR_W-1:0] REG_MATRIX_CAP0           = ADDR_W'(NPU_REG_MATRIX_CAP0);
    ...
    localparam logic [ADDR_W-1:0] REG_FAULT_CODE            = ADDR_W'(NPU_REG_FAULT_CODE);
    localparam logic [ADDR_W-1:0] REG_DEBUG_PC              = ADDR_W'(NPU_REG_DEBUG_PC);
    localparam logic [ADDR_W-1:0] REG_PROGRAM_DESC_ADDR_LO  = ADDR_W'(NPU_REG_PROGRAM_DESC_ADDR_LO);
    localparam logic [ADDR_W-1:0] REG_PROGRAM_DESC_ADDR_HI  = ADDR_W'(NPU_REG_PROGRAM_DESC_ADDR_HI);
    ...
    localparam logic [ADDR_W-1:0] REG_IRQ_CLEAR             = ADDR_W'(NPU_REG_IRQ_CLEAR);
    localparam logic [ADDR_W-1:0] REG_PERF_CYCLE_LO         = ADDR_W'(NPU_REG_PERF_CYCLE_LO);
    localparam logic [ADDR_W-1:0] REG_PERF_CYCLE_HI         = ADDR_W'(NPU_REG_PERF_CYCLE_HI);
    localparam logic [ADDR_W-1:0] REG_PERF_INSTRET_LO       = ADDR_W'(NPU_REG_PERF_INSTRET_LO);
    localparam logic [ADDR_W-1:0] REG_PERF_INSTRET_HI       = ADDR_W'(NPU_REG_PERF_INSTRET_HI);
    ...
    assign reset_clear_o = soft_reset_q && reset_quiescent_i;
    ...
    assign irq_pending_masked = irq_enable_q & irq_status_q & NPU_IRQ_VALID_MASK;
    assign irq_o = |irq_pending_masked;
    assign status_value = lifecycle_q | (irq_o ? NPU_STATUS_IRQ_PENDING : 32'h0000_0000);
    assign lifecycle_active = (lifecycle_q == NPU_STATUS_LOADING) ||
                              (lifecycle_q == NPU_STATUS_RUNNING) ||
                              (lifecycle_q == NPU_STATUS_HALTED);
    assign lifecycle_terminal = (lifecycle_q == NPU_STATUS_DONE) ||
                                (lifecycle_q == NPU_STATUS_FAULT);
    ...
    task automatic reset_regs;
        begin
            desc_addr_lo_q <= NPU_RESET_PROGRAM_DESC_ADDR_LO;
            desc_addr_hi_q <= NPU_RESET_PROGRAM_DESC_ADDR_HI;
            irq_enable_q <= NPU_RESET_IRQ_ENABLE;
            irq_status_q <= NPU_RESET_IRQ_STATUS;
            lifecycle_q <= NPU_STATUS_IDLE;
            fault_code_q <= NPU_RESET_FAULT_CODE;
            debug_pc_q <= NPU_RESET_DEBUG_PC;
    ...
    always_ff @(posedge s_axil.aclk_i or negedge s_axil.aresetn_i) begin
        if (!s_axil.aresetn_i) begin
            ...
            if (soft_reset_q && reset_quiescent_i) begin
                reset_regs();
                soft_reset_q <= 1'b0;
                halt_request_q <= 1'b0;
                clear_perf_q <= 1'b1;
            ...
            if (frontend_fault_i && lifecycle_active) begin
                ...
                if (irq_on_fault_i) begin
                    irq_status_q <= irq_status_q | NPU_IRQ_FAULT;
                end
            end else if (frontend_done_i && lifecycle_active) begin
                halt_request_q <= 1'b0;
                lifecycle_q <= NPU_STATUS_DONE;
                fault_code_q <= NPU_FAULT_NONE;
                debug_pc_q <= frontend_debug_pc_i;
                if (irq_on_done_i) begin
                    irq_status_q <= irq_status_q | NPU_IRQ_DONE;
                end
            end else if (frontend_halted_i && !resume_q &&
                         (lifecycle_q == NPU_STATUS_RUNNING)) begin
                halt_request_q <= 1'b0;
                lifecycle_q <= NPU_STATUS_HALTED;
                debug_pc_q <= frontend_debug_pc_i;
                irq_status_q <= irq_status_q | NPU_IRQ_HALTED;
            end else if (loader_done_i && (lifecycle_q == NPU_STATUS_LOADING)) begin
                lifecycle_q <= NPU_STATUS_RUNNING;
            ...
            if (frontend_halted_i && (lifecycle_q == NPU_STATUS_HALTED)) begin
                debug_pc_q <= frontend_debug_pc_i;
            end
                ...
                unique case (write_addr)
                    REG_CONTROL: begin
                        if ((write_strb != FULL_STRB) ||
                            ((write_data & ~NPU_CONTROL_VALID_MASK) != 32'h0000_0000) ||
                            has_multiple_bits(write_data & NPU_CONTROL_VALID_MASK)) begin
                            bresp_q <= AXI_RESP_SLVERR;
                        end else begin
                            if ((write_data & NPU_CONTROL_SOFT_RESET) != 32'h0000_0000) begin
                                if (lifecycle_q == NPU_STATUS_RESETTING) begin
                                    bresp_q <= AXI_RESP_SLVERR;
                                end else begin
                                    lifecycle_q <= NPU_STATUS_RESETTING;
                                    soft_reset_q <= 1'b1;
                                    halt_request_q <= 1'b0;
                                end
                            end else begin
                                if ((write_data & NPU_CONTROL_CLEAR_TERMINAL) != 32'h0000_0000) begin
                                ...
                                if ((write_data & NPU_CONTROL_HALT) != 32'h0000_0000) begin
                                    if (lifecycle_q == NPU_STATUS_RUNNING) begin
                                        halt_request_q <= 1'b1;
                                    end else begin
                                        bresp_q <= AXI_RESP_SLVERR;
                                    end
                                end
                                if ((write_data & NPU_CONTROL_RESUME) != 32'h0000_0000) begin
                                    if (lifecycle_q == NPU_STATUS_HALTED) begin
                                        halt_request_q <= 1'b0;
                                        lifecycle_q <= NPU_STATUS_RUNNING;
                                        resume_q <= 1'b1;
                                    end else begin
                                ...
                                if ((write_data & NPU_CONTROL_DEBUG_STEP) != 32'h0000_0000) begin
                                    if (lifecycle_q == NPU_STATUS_HALTED) begin
                                        debug_step_q <= 1'b1;
                                        irq_status_q <= irq_status_q | NPU_IRQ_DEBUG_STEP;
                                    end else begin
                                        bresp_q <= AXI_RESP_SLVERR;
                                    end
                                end
                    ...
                    REG_DOORBELL: begin
                        if ((write_strb != FULL_STRB) ||
                            ((write_data & ~NPU_DOORBELL_VALID_MASK) != 32'h0000_0000)) begin
                            bresp_q <= AXI_RESP_SLVERR;
                        end else if ((write_data & NPU_DOORBELL_START) != 32'h0000_0000) begin
                            if (lifecycle_q != NPU_STATUS_IDLE) begin
                                bresp_q <= AXI_RESP_SLVERR;
                                ...
                                halt_request_q <= 1'b0;
                                lifecycle_q <= NPU_STATUS_LOADING;
                                fault_code_q <= NPU_FAULT_NONE;
                                debug_pc_q <= 32'h0000_0000;
                    ...
                    REG_IRQ_ENABLE: begin
                        if ((apply_wstrb(irq_enable_q, write_data, write_strb) &
                             ~NPU_IRQ_VALID_MASK) != 32'h0000_0000) begin
                            bresp_q <= AXI_RESP_SLVERR;
                        end else begin
                    ...
                    REG_IRQ_CLEAR: begin
                        if ((write_strb != FULL_STRB) ||
                            ((write_data & ~NPU_IRQ_VALID_MASK) != 32'h0000_0000)) begin
                            bresp_q <= AXI_RESP_SLVERR;
                        end else begin
            ...
            if (read_fire) begin
                ...
                unique case (s_axil.araddr)
                    REG_DEVICE_ID: rdata_q <= NPU_RESET_DEVICE_ID;
                    REG_ABI_VERSION: rdata_q <= NPU_RESET_ABI_VERSION;
                    REG_ISA_VERSION: rdata_q <= NPU_RESET_ISA_VERSION;
                    REG_CAP0_LO: rdata_q <= NPU_RESET_CAP0_LO;
                    REG_CAP0_HI: rdata_q <= NPU_RESET_CAP0_HI;
                    REG_OP_CLASS_LO: rdata_q <= NPU_RESET_OP_CLASS_LO;
                    REG_OP_CLASS_HI: rdata_q <= NPU_RESET_OP_CLASS_HI;
                    REG_PROGRAM_MEM_BYTES: rdata_q <= NPU_RESET_PROGRAM_MEM_BYTES;
                    REG_LOCAL_MEM_BYTES: rdata_q <= NPU_RESET_LOCAL_MEM_BYTES;
                    REG_VECTOR_CAP0: rdata_q <= NPU_RESET_VECTOR_CAP0;
                    REG_MATRIX_CAP0: rdata_q <= NPU_RESET_MATRIX_CAP0;
                    REG_CONTROL: rdata_q <= NPU_RESET_CONTROL;
                    ...
                    REG_DOORBELL: rdata_q <= NPU_RESET_DOORBELL;
                    REG_IRQ_ENABLE: rdata_q <= irq_enable_q;
                    REG_IRQ_STATUS: rdata_q <= irq_status_q & NPU_IRQ_VALID_MASK;
                    REG_IRQ_CLEAR: rdata_q <= NPU_RESET_IRQ_CLEAR;
     ...
     control_lifecycle_onehot: assert property (
     ...
     control_resetting_matches_request: assert property (
        @(posedge s_axil.aclk_i) disable iff (!s_axil.aresetn_i)
            (lifecycle_q == NPU_STATUS_RESETTING) == soft_reset_q
    );
     control_reset_completes_idle: assert property (
        @(posedge s_axil.aclk_i) disable iff (!s_axil.aresetn_i)
            reset_clear_o |=> lifecycle_q == NPU_STATUS_IDLE
    );
     control_resetting_seen: cover property (
        @(posedge s_axil.aclk_i) disable iff (!s_axil.aresetn_i)
            lifecycle_q == NPU_STATUS_RESETTING
    );
     control_irq_output_matches_state: assert property (
        @(posedge s_axil.aclk_i) disable iff (!s_axil.aresetn_i)
            irq_o == |(irq_enable_q & irq_status_q & NPU_IRQ_VALID_MASK)
    );
     control_doorbell_active_rejected: assert property (
        @(posedge s_axil.aclk_i) disable iff (!s_axil.aresetn_i)
            write_fire && (write_addr == REG_DOORBELL) &&
            (write_strb == FULL_STRB) && (write_data == NPU_DOORBELL_START) &&
            (lifecycle_q != NPU_STATUS_IDLE)
