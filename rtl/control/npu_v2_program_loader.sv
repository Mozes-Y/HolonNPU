...
module npu_v2_program_loader_core #(
    ...
    npu_v2_localmem_wr_if.master    program_wr,
    ...
    assign m_axi.arlen = (phase_q == PHASE_DESC) ? 8'(DESC_BEATS - 1) : 8'(copy_ar_words - 5'd1);
            ...
            validation_fault_code = NPU_V2_FAULT_UNSUPPORTED_OPERATION_CLASS;
                     ...
                     ((desc_completion_addr != 64'h0000_0000_0000_0000) &&
                      !range_fits_u64(
                          ...
                          32'(NPU_V2_COMPLETION_RECORD_SIZE)
                      ))) begin
            validation_fault = 1'b1;
            validation_fault_code = NPU_V2_FAULT_INVALID_PROGRAM_DESCRIPTOR;
    ...
    v2_loader_copy_burst_profile: assert property (
        @(posedge clk_i) disable iff (!rst_ni)
    ...
    v2_loader_program_write_in_bounds: assert property (
        @(posedge clk_i) disable iff (!rst_ni)
            program_wr.req_valid |-> (program_wr.req_addr < desc_program_mem_bytes)
    );
    v2_loader_data_write_in_bounds: assert property (
        @(posedge clk_i) disable iff (!rst_ni)
