...
module npu_control_plane_core #(
    ...
    npu_localmem_wr_if.slave   data_client_wr,
    npu_localmem_rd_if.slave   data_client_rd,
    ...
    npu_localmem_wr_if data_arb_wr_if (
        .clk_i(s_axil.aclk_i),
        .rst_ni(s_axil.aresetn_i)
    );
    ...
    assign merged_fault = !control_soft_reset &&
                          (loader_fault_for_control || local_memory_fault || frontend_fault_i);
    assign merged_fault_code = loader_fault_for_control ? loader_fault_code_q :
                               local_memory_fault ? NPU_FAULT_LOCAL_MEMORY_BOUNDS :
    ...
    assign irq_on_done = (flags_o & NPU_PROGRAM_FLAG_IRQ_ON_DONE) != 32'd0;
    assign irq_on_fault = !frontend_event_uses_program_flags ||
                          ((flags_o & NPU_PROGRAM_FLAG_IRQ_ON_FAULT) != 32'd0);
    assign debug_snapshot_on_fault = frontend_event_uses_program_flags &&
        ((flags_o & NPU_PROGRAM_FLAG_DEBUG_SNAPSHOT_ON_FAULT) != 32'd0);
    assign clear_perf_on_start =
        (flags_o & NPU_PROGRAM_FLAG_CLEAR_PERF_ON_START) != 32'd0;
    ...
    assign reset_clear_o = control_reset_clear;
    ...
    always_ff @(posedge s_axil.aclk_i or negedge s_axil.aresetn_i) begin
        if (!s_axil.aresetn_i) begin
            ...
            loader_done_q <= 1'b0;
            loader_fault_q <= 1'b0;
            loader_fault_code_q <= NPU_FAULT_NONE;
    ...
    npu_control_regs_core u_control (
        .s_axil(s_axil),
        ...
        .reset_quiescent_i(reset_quiescent_i),
    ...
    npu_local_memory_core u_local_memory (
        .clk_i(s_axil.aclk_i),
        .rst_ni(s_axil.aresetn_i),
        .soft_reset_i(control_reset_clear),
        .program_wr(program_wr_if),
     ...
     control_plane_loader_start_profile: assert property (
        @(posedge s_axil.aclk_i) disable iff (!s_axil.aresetn_i)
            loader_start |-> !loader_busy_o
    );
     control_plane_loader_fault_has_code: assert property (
        @(posedge s_axil.aclk_i) disable iff (!s_axil.aresetn_i)
            loader_fault_q |-> (loader_fault_code_q != NPU_FAULT_NONE)
    );
     control_plane_local_memory_fault_has_code: assert property (
        @(posedge s_axil.aclk_i) disable iff (!s_axil.aresetn_i)
            local_memory_fault |-> (merged_fault_code == NPU_FAULT_LOCAL_MEMORY_BOUNDS)
    );
