...
module npu_v2_frontend_tile_core #(
    ...
    npu_v2_localmem_wr_if dma_data_wr_if (
        .clk_i(s_axil.aclk_i),
        .rst_ni(s_axil.aresetn_i)
    );

    npu_v2_localmem_wr_if vector_data_wr_if (
        .clk_i(s_axil.aclk_i),
        .rst_ni(s_axil.aresetn_i)
    );

    npu_v2_localmem_wr_if matrix_data_wr_if (
        .clk_i(s_axil.aclk_i),
        .rst_ni(s_axil.aresetn_i)
    );

    npu_v2_localmem_wr_if scalar_data_wr_if (
        .clk_i(s_axil.aclk_i),
        .rst_ni(s_axil.aresetn_i)
    );

    npu_v2_localmem_wr_if engine_data_wr_if (
        .clk_i(s_axil.aclk_i),
        .rst_ni(s_axil.aresetn_i)
    );
    ...
    npu_v2_localmem_rd_if dma_data_rd_if (
        .clk_i(s_axil.aclk_i),
        .rst_ni(s_axil.aresetn_i)
    );

    npu_v2_localmem_rd_if vector_data_rd_if (
        .clk_i(s_axil.aclk_i),
        .rst_ni(s_axil.aresetn_i)
    );

    npu_v2_localmem_rd_if matrix_data_rd_if (
        .clk_i(s_axil.aclk_i),
        .rst_ni(s_axil.aresetn_i)
    );

    npu_v2_localmem_rd_if scalar_data_rd_if (
        .clk_i(s_axil.aclk_i),
        .rst_ni(s_axil.aresetn_i)
    );

    npu_v2_localmem_rd_if engine_data_rd_if (
        .clk_i(s_axil.aclk_i),
        .rst_ni(s_axil.aresetn_i)
    );
    ...
    always_ff @(posedge s_axil.aclk_i or negedge s_axil.aresetn_i) begin
        if (!s_axil.aresetn_i) begin
            ...
            terminal_fault_code_q <= NPU_V2_FAULT_NONE;
            ...
            if (frontend_start) begin
                ...
                terminal_debug_pc_q <= frontend_if.fault &&
                    ((program_flags & NPU_V2_PROGRAM_FLAG_DEBUG_SNAPSHOT_ON_FAULT) == 32'd0)
    ...
    npu_v2_axi_write_arbiter_core u_write_arbiter (
        .clk_i(s_axil.aclk_i),
        .rst_ni(s_axil.aresetn_i),
        .soft_reset_i(control_soft_reset),
        .dma(dma_axi_if),
    ...
    v2_completion_precedes_control_terminal: assert property (
        @(posedge s_axil.aclk_i) disable iff (!s_axil.aresetn_i || control_soft_reset)
            (completion_addr != 64'd0) && (control_frontend_done || control_frontend_fault)
            |-> completion_done || completion_fault
    );
    v2_completion_writer_not_restarted_while_pending: assert property (
        @(posedge s_axil.aclk_i) disable iff (!s_axil.aresetn_i || control_soft_reset)
    ...
    npu_v2_vector_engine_core u_vector_engine (
        .clk_i(s_axil.aclk_i),
        .rst_ni(s_axil.aresetn_i),
        .soft_reset_i(control_soft_reset),
    ...
    npu_v2_matrix_engine_core u_matrix_engine (
        .clk_i(s_axil.aclk_i),
        .rst_ni(s_axil.aresetn_i),
        .soft_reset_i(control_soft_reset),
        .local_mem_bytes_i(local_mem_bytes),
    ...
    npu_v2_engine_data_arbiter_core u_engine_data_arbiter (
        .clk_i(s_axil.aclk_i),
        .rst_ni(s_axil.aresetn_i),
        .soft_reset_i(control_soft_reset),
    ...
    v2_frontend_tile_starts_after_loader_done: assert property (
        @(posedge s_axil.aclk_i) disable iff (!s_axil.aresetn_i || control_soft_reset)
            frontend_start |-> loader_done_o && !loader_fault_o
    );
    v2_frontend_tile_no_loader_frontend_double_fault: assert property (
        @(posedge s_axil.aclk_i) disable iff (!s_axil.aresetn_i || control_soft_reset)
            loader_fault_o |-> !frontend_start
    );
    v2_frontend_tile_program_fetch_after_start: cover property (
        @(posedge s_axil.aclk_i) disable iff (!s_axil.aresetn_i || control_soft_reset)
            frontend_if.program_rd_valid
    );
    v2_frontend_tile_read_owner_stable: assert property (
        @(posedge s_axil.aclk_i) disable iff (!s_axil.aresetn_i || control_soft_reset)
            (read_owner_q != READ_OWNER_NONE) && !(m_axi.rvalid && m_axi.rready && m_axi.rlast)
            |=> read_owner_q == $past(read_owner_q)
    );
    v2_frontend_tile_dma_write_seen: cover property (
        @(posedge s_axil.aclk_i) disable iff (!s_axil.aresetn_i || control_soft_reset)
            dma_data_wr_if.req_valid && dma_data_wr_if.req_ready
    );
    v2_frontend_tile_vector_issue_seen: cover property (
        @(posedge s_axil.aclk_i) disable iff (!s_axil.aresetn_i || control_soft_reset)
            vector_issue_if.valid && vector_issue_if.ready
    );
    v2_frontend_tile_vector_result_seen: cover property (
        @(posedge s_axil.aclk_i) disable iff (!s_axil.aresetn_i || control_soft_reset)
            vector_result_if.valid && vector_result_if.ready
    );
    v2_frontend_tile_matrix_result_seen: cover property (
        @(posedge s_axil.aclk_i) disable iff (!s_axil.aresetn_i || control_soft_reset)
            matrix_result_if.valid && matrix_result_if.ready
    );
    v2_frontend_tile_scalar_memory_seen: cover property (
        @(posedge s_axil.aclk_i) disable iff (!s_axil.aresetn_i || control_soft_reset)
            (scalar_data_rd_if.req_valid && scalar_data_rd_if.req_ready) ||
            (scalar_data_wr_if.req_valid && scalar_data_wr_if.req_ready)
    );
    v2_frontend_tile_sync_issue_valid: assert property (
        @(posedge s_axil.aclk_i) disable iff (!s_axil.aresetn_i || control_soft_reset)
    ...
    v2_frontend_tile_sync_issue_seen: cover property (
        @(posedge s_axil.aclk_i) disable iff (!s_axil.aresetn_i || control_soft_reset)
