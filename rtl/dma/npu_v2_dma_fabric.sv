...
module npu_v2_dma_fabric_core #(
    ...
    assign burst_words = (bytes_remaining_q > 32'(COPY_BURST_MAX_WORDS * WORD_BYTES)) ?
                         5'(COPY_BURST_MAX_WORDS) :
                         5'(bytes_remaining_q[6:2]);
    ...
    assign frontend.dma_issue_ready = (state_q == STATE_IDLE);
        ...
        if (!frontend.rst_ni) begin
            ...
            unique case (state_q)
                ...
                STATE_STORE_B: begin
                    if (b_fire) begin
                        if (!b_resp_ok) begin
                            fault_code_q <= NPU_V2_FAULT_AXI_WRITE;
                            state_q <= STATE_FAULT;
    ...
    v2_dma_write_burst_profile: assert property (
        @(posedge frontend.clk_i) disable iff (!frontend.rst_ni || frontend.soft_reset)
            m_axi.awvalid |->
    ...
    v2_dma_local_write_in_bounds: assert property (
        @(posedge frontend.clk_i) disable iff (!frontend.rst_ni || frontend.soft_reset)
            data_wr.req_valid |-> (data_wr.req_addr < frontend.local_mem_bytes)
    );
    v2_dma_local_read_in_bounds: assert property (
        @(posedge frontend.clk_i) disable iff (!frontend.rst_ni || frontend.soft_reset)
            data_rd.req_valid |-> (data_rd.req_addr < frontend.local_mem_bytes)
    );
    v2_dma_store_uses_word_strobe: assert property (
        @(posedge frontend.clk_i) disable iff (!frontend.rst_ni || frontend.soft_reset)
            m_axi.wvalid |-> (m_axi.wstrb != '0)
    );
    v2_dma_success_seen: cover property (
        @(posedge frontend.clk_i) disable iff (!frontend.rst_ni || frontend.soft_reset)
            frontend.dma_event_valid
    );
    v2_dma_fault_seen: cover property (
        @(posedge frontend.clk_i) disable iff (!frontend.rst_ni || frontend.soft_reset)
