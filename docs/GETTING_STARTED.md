...
2. load A tile row。
3. load B tile row。
4. 让 systolic array 计算。
5. 如果 K 还有后续 tile，继续累加。
...
v1 使用 output-stationary dataflow：

- 每个 PE 保存一个 C partial sum。
- A 从左侧输入，向右传播。
- B 从上侧输入，向下传播。
- 每个 PE 做 signed INT8 乘法，然后累加到 signed INT32。
...
- tile mask。
- A/B tile buffer。
- C accumulation/output buffer。
- banked scratchpad。
- ping-pong schedule。
- 给 systolic array 提供正确时序的 A/B 输入。
...
```sh
ctest --preset debug -R npu_top --output-on-failure
```

只跑 GEMM：

```sh
ctest --preset debug -R npu_gemm --output-on-failure
```

只跑 DMA：

```sh
ctest --preset debug -R 'npu_read_dma|npu_write_dma' --output-on-failure
```
...
ctest --preset debug -R npu_control --output-on-failure
...
ctest --preset debug -R 'npu_pe|npu_array|npu_gemm|npu_top' --output-on-failure
cmake --build --preset debug --target matrix_rtl_lint integration_rtl_lint
```

特别注意：

- A-left/B-top 边界输入契约不能随便改。
- valid/mask 传播必须保持正确。
...
ctest --preset debug -R 'npu_gemm|npu_top' --output-on-failure
...
5. 必要时打开 wave。
6. 修复后先跑局部测试。
7. 再跑完整 `ctest --preset debug`。
8. 最后跑 `v1_lint` 和 regression。
...
- Verilator
- Python 3
- 通过 `pip` 安装的 CMake 4.x

这里特意不用 runner 自带的旧 CMake，因为项目 `CMakePresets.json` 明确要求
CMake 4.0+。CI 也显式使用 `g++-14`，避免 C++26 feature 检测因为默认编译器
过旧而失败。
