...
- V2.2 has started with a stdlib-only C++26 architectural simulator foundation
  ...
  `i32` vector load/store/add/sub/min/max, matrix `i8*i8->i32` micro-op
  effects, and focused fault paths.
...
| V2 model test | `ctest --preset debug -R holon_npu_v2_model --output-on-failure` | Passed `1/1`, including ABI 3.0 descriptor loading, DMA events, predicate vector ALU, and matrix micro-op effects |
...
- Continue V2 Phase V2.2 by expanding the C++ architectural simulator with
  additional vector operation classes, deterministic random program generation,
